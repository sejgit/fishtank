...
# add the rotating log message handler
tfh = logging.handlers.RotatingFileHandler(TEMP_LOG_FILENAME,
                                           maxBytes=0,
                                           backupCount=5)
...
def prowl(event, description, pri=None):
    ...
    try:
        ...
        # prowl push to sej
        p.push(apikey1, fishlabel, event, description, url=None, priority=pri)

        # prowl push to tej
        p.push(apikey2, fishlabel, event, description, url=None, priority=pri)

        # prowl push to csj
        p.push(apikey3, fishlabel, event, description, url=None, priority=pri)
        success = True
    except IOError:
...
def pushtempstatus():
    ...
    if status != pushtempstatus.status_old:
        ...
        try:  # status
            s = (isyip + "/rest/vars/set/2/" + str(args.isy + 2) + "/" +
                 str(int(status == "ok")))
            r = requests.get(s, auth=(isylogin, isypass))
            if r.status_code != requests.codes.ok:
                logger.error("isy update temp error =" + str(r.status_code))
        except Exception:
...
def dailylog():
    deg_c, deg_f, status = read_temp()
    logger.info("celcius {0:.2f}  fahrenheit {1:.2f}  {2}".format(
        deg_c, deg_f, status))
    templogger.info("{2}, {0:.2f}, {1:.2f}".format(deg_c, deg_f, status))
    try:
        aio.send(args.stream, deg_f)
        ...
        try:
            ...
            if not args.test:
                with open(dir + "user_annotate.txt", "w") as f:
                    f.write("celcius {0:.2f}  fahrenheit {1:.2f}  {2}".format(
                        deg_c, deg_f, status + hb))
                f.closed
